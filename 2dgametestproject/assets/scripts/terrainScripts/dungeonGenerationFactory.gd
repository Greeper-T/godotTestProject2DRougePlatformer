...
var wall_tile_bottom := Vector2i(9,1)
	...
	for x in range(WIDTH):
		for y in range(HEIGHT):
			...
			if grid[x][y] == 1:
				...
				draw_tile_color(Vector2i(x, y), Color.hex(0x222222FF))
			else:
				tile_map_layer.set_cell(tile_position, 0, Vector2(-1, -1))

func draw_tile_color(grid_pos: Vector2i, color: Color):
	var rect = ColorRect.new()
	rect.color = color
	rect.size = Vector2(CELL_SIZE, CELL_SIZE)
	rect.position = grid_pos * CELL_SIZE
	add_child(rect)
